...
namespace BookAPI.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        {
            var maKh = User.FindFirst(ClaimTypes.Email)?.Value;
            _logger.LogInformation("Yêu cầu lấy chi tiết hóa đơn của khách hàng {maKh} có mã HD {maHD}", maKh, id);
            var ordersDetail = await _hoaDonCT.GetAllAsync(id);
            _logger.LogInformation("Yêu cầu lấy chi tiết hóa đơn của khách hàng {maKh} có mã HD {maHD} thành công", maKh, id);
            return Ok(new ApiResponse
            {
                Success = true,
        ...
        public async Task<IActionResult> OrdersConfirm(int? page, int? pageSize)
        {
            ...
            var result = await _hoaDon.GetOderConfirm(maKh, _page, _pageSize);
